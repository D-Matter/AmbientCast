...
# AmbientCast 0.17.1 (Jun 16, 2022)
...
- **LetsEncrypt via the Web**: We now support configuring LetsEncrypt via the web interface. If you had previously set
  ...
  been updated to the following:
    - Full chain certificate: `/var/ambientcast/acme/ssl.crt`
    - Private Key: `/var/ambientcast/acme/ssl.key`
...
# AmbientCast 0.17.0 (Jun 6, 2022)

## New Features/Changes

- **HLS Support**: We now support the HTTP Live Streaming (HLS) format from directly within the AmbientCast web UI. Once
...
- We have updated AmbientCast's AutoDJ scheduler to be able to handle the "Advanced" playlist configuration options
...
- Since AmbientCast's services are all now accessible via `localhost`, several connections have been switched from TCP/IP
...
- Docker users can now debug Slim Application Errors by editing the `SHOW_DETAILED_ERRORS` in the `ambientcast.env` file,
  reports should be submitted to our [issues](https://github.com/ambientcast/ambientcast/issues) section for review by our
  team.
...
# AmbientCast 0.16.1 (May 03, 2022)
...
# AmbientCast 0.16.0 (Apr 27, 2022)

## New Features/Changes

- AmbientCast can now process new media files, including ScreamTracker Modules (.stm, .s3m), Module/Extended Modules
...
- **Unified Docker Container**: We have combined all of our Docker containers into a single unified container that
  ...
  at [dmatterhub/ambientcast](https://github.com/ambientcast/AmbientCast/pkgs/container/ambientcast). For most users, no
  changes will be needed when migrating to the latest version of AmbientCast, but if you have created
  a `docker-compose.override.yml` file, you should
  follow [our instructions](https://github.com/AmbientCast/AmbientCast/issues/5191) to update the file.
...
- Internal API requests (Icecast listener auth, Liquidsoap API calls) are now handled via a separate, dedicated "back
  channel" and won't be affected as severely by heavy traffic on AmbientCast from public viewers or administrators.

- When processing media, we now use a combination of two libraries (php-getid3 and ffmpeg/ffprobe) to process media and
  retrieve rich metadata; this greatly expands the types of media that we can handle in AmbientCast to include essentially
...
# AmbientCast 0.15.2 (Feb 20, 2022)
...
- Fixed a bug when reinstalling AmbientCast on top of an existing (or previous) installation.

---

# AmbientCast 0.15.1 (Feb 18, 2022)
...
- For Docker installations, we have removed our built-in multisite configuration in favor of a simpler default
  ...
  the [instructions in our documentation](https://docs.ambientcast.com/en/administration/docker/multi-site-installation).
...
- Updated to Liquidsoap version 2.0.3 on Ansible and Docker, this change includes some stability fixes and a patch for a
  ...
  megathread for more information [#5017](https://github.com/AmbientCast/AmbientCast/issues/5017)

## Bug Fixes

- Fixed a bug where station base directories created with relative names would end up in `/var/ambientcast/www/web`.
...
# AmbientCast 0.15.0 (Jan 12, 2022)
...
- **Vue Components Everywhere**: As part of our Roadmap to 1.0, we've switched a vast majority of the AmbientCast
...
- Changes have also been made to backend software libraries as part of our development progress:
    - Removed and retired `AmbientForms` library
    - Added [doctrine-entity-normalizer](https://github.com/AmbientCast/doctrine-entity-normalizer)
    - Added [doctrine-batch-utilities](https://github.com/AmbientCast/doctrine-batch-utilities)
...
# AmbientCast 0.14.1 (Aug 22, 2021)
...
- The entire AmbientCast codebase has been set to "strict mode" in PHP, which will strictly enforce type safety across the
...
- You can optionally disable Redis entirely, instead relying on flatfile caches for session management and other Redis
  functions by setting `ENABLE_REDIS=false` in `ambientcast.env`. This is not recommended for most users as Redis offers
...
- One of the biggest issues with Docker file mounting has been permissions; you can now set a custom UID/GID for the
  ...
  set `AMBIENTCAST_PUID` and `AMBIENTCAST_PGID` in `.env` accordingly; both default to 1000.

- All up-to-date AmbientCast installations will opt users out of Google's new advertisement tracking system, FLoC. Learn
...
# AmbientCast 0.13.0 (Jun 15, 2021)

## New Features/Changes

- **Podcast Management (Beta):** You can now upload and manage podcasts directly via the AmbientCast web interface. Via
...
- **Automatic Theme Selection:** If you haven't set a default theme for either your user account or the AmbientCast public
...
# AmbientCast 0.12.4 (Apr 27, 2021)
...
- The PHP-SPX (Simple Profiling eXtension) extension has been added and can be enabled via a new environment variable in
  `ambientcast.env`; this will allow very simple visual profiling of the application and its memory/CPU usage over time.
...
- AmbientCast can now properly read _and write_ metadata to/from Flac and Ogg Vorbis files without issue.
...
# AmbientCast 0.12.3 (Apr 14, 2021)
...
- For sequential or shuffled playlists, you can now view the internal queue that the AmbientCast AutoDJ uses to track its
...
- An issue with some stations crashing shortly after startup has been resolved. This was caused by a safety check we
  added to the AutoDJ to check that AmbientCast was up and running at the same time; however, this caused issues with
  stations that don't use the AmbientCast AutoDJ (i.e. stations that stream live or use remote playlists).
...
# AmbientCast 0.12.1 (Feb 19, 2021)
...
- Across all AmbientCast repositories, the `master` branch has been renamed to `main`.
...
# AmbientCast 0.12 (Jan 27, 2021)

This update introduces significant new features and fixes a number of bugs reported by the community.

## New Features/Changes

- **Remote Album Art Retrieval**: If enabled in the system settings panel, AmbientCast will now check remote services to
  attempt to retrieve album art if it is missing, or not provided (i.e. for live DJs). By default, this system uses the
  MusicBrainz database, which is comprehensive but can be slow; if you provide an API key for the last.fm API, AmbientCast
...
- **Media Manager Improvements:** Some changes have been made to the media manager to improve the user experience and
  accessibility:
    ...
    - You can now instruct AmbientCast to re-analyze and reprocess the selected media files in the Media Manager.
...
- Unprocessable media is now stored in a separate database table along with the date/time processed and the relevant
  ...
  processable but are processed in every 5-minute sync. AmbientCast will automatically re-check files marked as
...
- For more advanced setups, you can now set the following environment variables in `ambientcast.env` to use a third-party
  Redis service instead of the one bundled with AmbientCast:
    - `REDIS_HOST` (default: `redis` for Docker, `localhost` for Ansible)
    - `REDIS_PORT` (default: 6379)
    - `REDIS_DB` for the database index (default: 1)

- There is a new debug CLI command, `ambientcast:debug:optimize-tables`, which optimizes all tables in the MariaDB
...
# AmbientCast 0.11.2 (Dec 11, 2020)
...
- Previously AmbientCast's codebase had a `Settings` class and a `Settings` entity that managed two separate things, which
...
- There is a new, much friendlier animation that displays when Docker installations of AmbientCast are waiting for their
...
- **Removal of InfluxDB**: Despite the ever-increasing complexity of AmbientCast, sometimes we review our dependencies to
...
- **Songs table overhaul**: A `Songs` table has existed for the entirety of AmbientCast's existence as the authoritative
...
# AmbientCast 0.10.4 (Oct 1, 2020)

This release is being tagged to incorporate all of the feature improvements, bug fixes and other changes we've made to
AmbientCast over the last few months into a stable release just prior to our Hacktoberfest event. We expect the
...
- Raspberry Pi support has been restored, and AmbientCast's Ansible installation is once again compatible with Raspberry
...
- Fixed a bug where you could continue into AmbientCast without finishing setup. (#2958)
...
# AmbientCast 0.10.3 (Jun 21, 2020)
...
- We have merged the configuration from our "multi-site" setup into our core Docker Compose configuration. This means
  ...
  multiple domains alongside your AmbientCast installation. See issue #2855 for instructions on migrating your existing
...
- There is a new CLI command to list all accounts on an AmbientCast installation. For Docker users, this can be invoked
  via `./docker.sh cli ambientcast:account:list`.
...
# AmbientCast 0.10.2 (May 6, 2020)

This is a primarily bug fix oriented release that includes several important improvements. All users of AmbientCast are
...
- You can set a default album art URL on a per-station level; if one is not set at the station level, the system-wide
  one will be used instead (and if one is not provided there, the default AmbientCast album art image will be used
  instead).
...
- The AmbientCast homepage will now remind you if you haven't run any sort of backup (either manual or automated nightly)
...
- The "Cued" time in the upcoming song queue will now accurately reflect when AmbientCast expects the track to actually be
  played.
...
# AmbientCast 0.10.1 (Apr 26, 2020)
...
# AmbientCast 0.10.0 (Apr 26, 2020)

AmbientCast 0.10.0 is an incremental release with several bug fixes and improvements across the application. This version
introduces minor changes to the AmbientCast API (see below) and is tagged as a new minor release as a result.
...
- AmbientCast's AutoDJ will now build a queue of songs ahead of the current song. Instead of just seeing the "next" song,
...
- Several improvements have been made to scheduling and the AmbientCast AutoDJ in general (#2740, #2689, #2631). This is
...
- **Schedule View**: Now that both playlists and streamers are scheduled, you can use AmbientCast to produce a full
...
- **Record Live Broadcasts**: Thanks to several improvements within AmbientCast, we now track the start and end times for
...
- **New IP Geolocation Options**: With the changes to the MaxMind GeoLite database, we've abstracted out how we perform
  ...
  and AmbientCast will download and use the GeoLite database instead.

## Bug Fixes and Minor Improvements

- The `ambientcast_web_v2` Docker image is now directly tied to the main code repository, so rolling-release updates are
...
- The "Prefer Browser URL" and "Enable SFTP Server" settings have been tweaked so they will behave more sensibly, and
  will not interfere with portions of AmbientCast's web interface that make API calls.

- Several sections of AmbientCast (including, notably, the Duplicate Songs report and Radio Automation code) have been
...
- The AmbientCast AutoDJ's scheduling code has been significantly overhauled to be easier to maintain and to fix a number
...
- When previewing a music track or other finite-length file within the AmbientCast web interface, you can seek through the
...
# AmbientCast 0.9.8.1 (Jan 11, 2020)
...
- **Switched Built-in FTP Service to SFTP:** We encountered a number of issues with our previous built-in FTP service,
  ...
  modifiable by changing/adding `AMBIENTCAST_SFTP_PORT` in `.env` on the host) to handle all incoming connections, is a
  ...
  existing AmbientCast credentials.
...
- The mini music player inside the AmbientCast interface is now inline with the header menu instead of accessed via a
  dropdown menu.
...
# AmbientCast 0.9.7.1 (Oct 12, 2019)
...
# AmbientCast 0.9.7 (Oct 11, 2019)

## New Features

- **New Media Manager**: The Media Manager is one of the core parts of the AmbientCast experience, and we want it to be as
...
- You can now customize even more of the AmbientCast Docker installation parameters by modifying configuration lines in
  your local [ambientcast.env](https://github.com/AmbientCast/AmbientCast/blob/main/ambientcast.sample.env#L70-L80) file.

## Bug Fixes

- We have resolved a major issue with one of our third-party libraries that causes "No valid bundles" errors to appear
  any time AmbientCast attempts to connect to a secure URL. This can happen even on pages that don't seemingly make any
...
- This version includes a significant number of code quality improvements under the hood. You won't notice many of these
  ...
  but they are meant to make AmbientCast's code much more consistent and easier to maintain, which means faster bug fixes
  and more new features!

---

# AmbientCast 0.9.6.5 (Aug 27, 2019)

## New Features

- This incremental minor version release adds support for AmbientCast's built-in FTP server on Docker installations, which
  allows you to use your AmbientCast credentials to log in and manage media via a high-performance FTP server.

---

# AmbientCast 0.9.6.2 (Aug 27, 2019)
...
# AmbientCast 0.9.6.1 (Aug 16, 2019)
...
- Tweaked the AmbientCast AutoDJ's duplicate artist/title prevention algorithm to be more forgiving of playlists that
...
- Fixed a scenario where hundreds of "zombie processes" were being created by AmbientCast sending notification messages to
  the nchan websocket now-playing service.

- Fixed a bug where AmbientRelay relays kept being newly recreated every time the relay sent its "Now Playing" update to
  the parent server.
...
- When determining your external-facing IP address (which requires pinging a service that's not located on your
  installation, one of the few times we do this), we have switched to using our own AmbientCast Central server for this
...
# AmbientCast 0.9.6 (Jul 27, 2019)

It's only been just over a month since AmbientCast 0.9.5.1 was released, but we've made some very significant, very
important updates to the software in that time, especially in the fields of reliability and performance.

## New Features

- **The AmbientCast AutoDJ is Back**: A few versions ago, we had attempted to switch entirely to using Liquidsoap for our
  ...
  AmbientCast AutoDJ include:
    - Being able to see the next song that will play in the queue,
    - Applying playlist changes without reloading all of AmbientCast, and
    - Avoiding songs playing back-to-back with either the same title _or_ the same artist using our duplicate prevention
      system.

- **Introducing AmbientRelay**: This version of AmbientCast has built-in support for our new "relay-in-a-box"
  software [AmbientRelay](https://github.com/AmbientCast/AmbientRelay). Simply drop AmbientRelay onto a server that can run
  Docker, answer a few quick questions, and it will automatically connect to AmbientCast, detect all stations, and relay
  them all. It will also list itself as a relay on the "host" AmbientCast station, and report back listener and client
  details as well.
...
- **Two New NowPlaying API Sources**: Our most powerful and comprehensive set of data is all compiled into what we call
  ...
  new [Now Playing Data APIs Guide](https://docs.ambientcast.com/en/developers/apis/now-playing-data).
...
- The currently playing song is written atomically to `/var/ambientcast/stations/(station_name)/nowplaying.txt`, which is
  particularly useful for our [radio-video-stream](https://github.com/AmbientCast/radio-video-stream) example project.
...
- If you are leaving a page with a form on it that has unsaved changes, you will be prompted to confirm leaving the
  page.
- Web servers will now more aggressively cache the hashed static assets used in AmbientCast, improving performance.
...
# AmbientCast 0.9.5.1 (Jun 11, 2019)
...
- The Highcharts library has been completely replaced across the entire application. This is largely because of
  ...
  component from early AmbientCast development that required updating to be a fully free and open-source stack.
...
- Fixed a number of scenarios that caused the "This station is powered by AmbientCast" jingle to play despite a station
...
# AmbientCast 0.9.5 (May 14, 2019)

Work on AmbientCast never stops, and with the project increasing in popularity, we've been even busier than before. A lot
...
- **API Parity**: We've done a _lot_ of work to make all of the core functionality of AmbientCast available via our REST
  ...
  visit `your-ambientcast-url/api` for API documentation specific to your installed version that you can test in-browser
...
- Fixed #1405 and other bugs related to the new Liquidsoap AutoDJ process that prevented AmbientCast from getting proper "
...
- If AmbientCast detects that you've modified your installation locally (specifically your Ansible/"Traditional"
...
# AmbientCast 0.9.4.2 (Mar 31, 2019)
...
- When restarting a station, the AmbientCast error message won't play immediately, but it will play if your station has
  nothing to play.
...
# AmbientCast 0.9.4.1 (Mar 27, 2019)
...
AmbientCast AutoDJ mode have been merged. Liquidsoap handles the scheduling and playback of all "once every x" and
otherwise scheduled playlists, and AmbientCast's AutoDJ handles, by default, the "general rotation" playlists. If an error
occurs with AmbientCast's AutoDJ, Liquidsoap will manage general rotation playlists, too.
...
We have also made improvements to how Liquidsoap is notified of playlist changes from AmbientCast, so these notifications
...
- **"Jingle Mode" Playlists**: You can now tell AmbientCast to ignore metadata updates for a given playlist, so that when
...
- **Support for Two-Factor Authentication**: Now any user on AmbientCast can significantly improve their security by
  ...
  the [password reset instructions](https://github.com/AmbientCast/AmbientCast/blob/main/SUPPORT.md#reset-an-account-password)
...
- The Twitter web hook now has support for rate limiting, so you can tell AmbientCast to only send one Tweet every few
...
- The installation method formerly known as "Traditional" has been renamed to "Ubuntu Bare-Metal" installation. The
  term "traditional" conveyed to many people that this was the "normal" or preferred way of installing AmbientCast, when
...
- Further improvements have been made to the process of notifying AmbientCast of song changes, and the station profile
...
- Several new protections have been introduced to prevent users from deleting themselves, removing critical permissions
  ...
  running `[your cli command] ambientcast:account:set-administrator [your-email-address]`.
...
- The `station_watcher` secondary process has been removed on Docker installations, replaced by new scripting
  ...
  support for AmbientCast's rich metadata even when using "Manual AutoDJ" mode or "Advanced" playlists.
...
- Starting with this release, releases are now tracked on the Composer package
  manager's [Packagist repository](https://packagist.org/packages/ambientcast/ambientcast). We will be using Packagist to
...
- **Redesigned Station Profiles and Per-Station File Quotas**: The form you see when editing your station profile has
  been divided into new tabs for easier use. If you are a global administrator of a multi-tenant AmbientCast instance, you
...
- **Release-Only Updates**: If you prefer to update less frequently, you can now choose to only update to the latest
  tagged release of AmbientCast instead of using the typical "rolling release" schedule. Both `./update.sh --release`
...
- When viewing your API documentation (`your-ambientcast-site/api`), the documentation will allow you to test the
...
- In some previous versions, AmbientCast referred to a third-party CDN (CloudFlare's CDNJS) for many of its static assets
  for performance reasons. From this version forward, those assets have been moved back to the AmbientCast instance
  itself. This helps to preserve the self-contained nature of AmbientCast and avoid unnecessary downtime caused by
...
# AmbientCast 0.9.0 (Oct 8, 2018)

**Important note:** AmbientCast is a "rolling-release" product that is updated almost daily. If you're running AmbientCast,
...
- **Shoutcast 2 DNAS is no longer bundled with AmbientCast.** While Shoutcast has been a popular offering that was bundled
  with AmbientCast after significant demand, it has always been non-free, proprietary software, the only component in the
  AmbientCast stack that is not free and open-source. With the release of Shoutcast's
  new ["freemium" pricing structure](https://www.shoutcast.com/Pricing) has also come a new, more aggressive license
  associated with distribution of the software. As such, AmbientCast can no longer bundle Shoutcast 2 DNAS with new
...
- **Our support for Icecast is now even better suited for commercial radio stations.** Along with our withdrawal of
  ...
  version of AmbientCast, you will now see much more accurate information on your listeners, especially if you're using
...
- **The core "Now Playing" library is a standalone PHP library now.** We're always looking for new ways to give back to
  the open-source software community. Now, along with [AmbientForms](https://github.com/AmbientCast/ambientforms), we have
  spun off our [NowPlaying](https://github.com/AmbientCast/nowplaying) library to be a standalone component you can
...
- **AmbientCast now has early support for plugins.** Many of you have wanted to customize the internal workings of
  AmbientCast without needing to fork the main codebase and maintain your own copy. You can now do this with the help of
  our new plugin architecture, which lets you hook directly into the most important events that happen "under the hood"
  in AmbientCast without modifying the core code. Check out
  our [example plugin](https://github.com/AmbientCast/example-plugin) for a demonstration of what plugins can do.
...
- **AmbientCast has a new homepage!** Visit our simple yet informative new homepage, powered by VuePress,
  at [ambientcast.com](https://www.ambientcast.com/).
...
- When editing a station's profile, AmbientCast will automatically check if a port is in use by another station before
...
# AmbientCast 0.8.0 (Dec 15, 2017)

This release includes the cumulative development work done over the fall and winter of 2017 on the AmbientCast project. It
...
- **Broadcast to remote streams:** Running your AutoDJ from AmbientCast but broadcasting to a hosted radio server
...
- **Frontend dependencies relocated to CDNs**: Many of the frontend dependencies used by the web app have been moved and
  ...
  significantly lightened the size of the AmbientCast codebase.

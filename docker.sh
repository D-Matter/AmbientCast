...
# Get the current release channel for AmbientCast
get-release-channel() {
  local AMBIENTCAST_VERSION="latest"
  if [[ -f .env ]]; then
    .env --file .env get AMBIENTCAST_VERSION
    AMBIENTCAST_VERSION="${REPLY:-latest}"
  fi

  echo "$AMBIENTCAST_VERSION"
}
...
#
# Configure the ports used by AmbientCast.
#
setup-ports() {
  envfile-set "AMBIENTCAST_HTTP_PORT" "80" "Port to use for HTTP connections"
  envfile-set "AMBIENTCAST_HTTPS_PORT" "443" "Port to use for HTTPS connections"
  envfile-set "AMBIENTCAST_SFTP_PORT" "2022" "Port to use for SFTP connections"
}
...
setup-release() {
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/main/sample.env -o .env
  fi
  ...
  local AMBIENTCAST_VERSION="${OLD_RELEASE_CHANNEL}"

  if [[ $AMBIENTCAST_VERSION == "latest" ]]; then
    if ask "Your current release channel is 'Rolling Release'. Switch to 'Stable' release channel?" N; then
      AMBIENTCAST_VERSION="stable"
    fi
  elif [[ $AMBIENTCAST_VERSION == "stable" ]]; then
    if ask "Your current release channel is 'Stable'. Switch to 'Rolling Release' release channel?" N; then
      AMBIENTCAST_VERSION="latest"
    fi
  fi

  .env --file .env set AMBIENTCAST_VERSION=${AMBIENTCAST_VERSION}

  if [[ $AMBIENTCAST_VERSION != $OLD_RELEASE_CHANNEL ]]; then
...
check-install-requirements() {
  ...
  echo "Checking installation requirements for AmbientCast..."
  ...
  if [[ $CURRENT_OS == "Linux" ]]; then
    ...
    echo "       Automated AmbientCast installation is not currently supported on this"
  ...
  if [[ $SCRIPT_DIR == "/var/ambientcast" ]]; then
    echo -en "\e[32m[PASS]\e[0m Installation Directory\n"
  else
    echo -en "\e[93m[WARN]\e[0m Installation Directory\n"
    echo "       AmbientCast is not installed in /var/ambientcast, as is recommended"
    echo "       for most installations. This will not prevent AmbientCast from"
...
run-installer() {
  local AMBIENTCAST_RELEASE_BRANCH
  AMBIENTCAST_RELEASE_BRANCH=$(get-release-branch-name)

  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/sample.env -o .env
  fi
  if [[ ! -f ambientcast.env ]]; then
    curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/ambientcast.sample.env -o ambientcast.env
  fi
  if [[ ! -f docker-compose.yml ]]; then
    curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.yml
  fi
  ...
  curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/docker-compose.installer.yml -o docker-compose.installer.yml

  docker-compose -p ambientcast_installer -f docker-compose.installer.yml pull
  docker-compose -p ambientcast_installer -f docker-compose.installer.yml run --rm installer install "$@"

  rm docker-compose.installer.yml
}

#
# Run the initial installer of Docker and AmbientCast.
...
install() {
  ...
  if [[ $EUID -ne 0 ]]; then
    .env --file .env set AMBIENTCAST_PUID="$(id -u)"
    .env --file .env set AMBIENTCAST_PGID="$(id -g)"
  fi
...
install-dev() {
  ...
  if [[ ! -f ambientcast.env ]]; then
    ...
    echo "Customize ambientcast.env file now before continuing. Re-run this command to continue installation."
    exit
  fi
  ...
  docker-compose -p ambientcast_frontend -f docker-compose.frontend.yml run --rm frontend npm run build
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.new.yml
    fi
    ...
    docker-compose run --rm web -- ambientcast_update "$@"
    docker-compose up -d
...
update-self() {
  ...
  curl -fsSL https://raw.githubusercontent.com/D-Matter/AmbientCast/$AMBIENTCAST_RELEASE_BRANCH/docker.sh -o docker.sh
  chmod a+x docker.sh
...
#
cli() {
  docker-compose exec --user="ambientcast" web ambientcast_cli "$@"
  exit
}
...
db() {
  ...
  .env --file ambientcast.env get MYSQL_HOST
  MYSQL_HOST="${REPLY:-localhost}"

  .env --file ambientcast.env get MYSQL_PORT
  MYSQL_PORT="${REPLY:-3306}"

  .env --file ambientcast.env get MYSQL_USER
  MYSQL_USER="${REPLY:-ambientcast}"

  .env --file ambientcast.env get MYSQL_PASSWORD
  MYSQL_PASSWORD="${REPLY:-ambient4c457}"

  .env --file ambientcast.env get MYSQL_DATABASE
  MYSQL_DATABASE="${REPLY:-ambientcast}"
...
backup() {
  ...
  docker-compose exec --user="ambientcast" web ambientcast_cli ambientcast:backup "/var/ambientcast/backups/${BACKUP_FILENAME}" "$@"

  # Move from Docker volume to local filesystem
  docker run --rm -v "ambientcast_backups:/backup_src" \
  -v "$BACKUP_DIR:/backup_dest" \
  busybox mv "/backup_src/${BACKUP_FILENAME}" "/backup_dest/${BACKUP_FILENAME}"
}

#
# Restore an AmbientCast backup into Docker.
# Usage:
# ./docker.sh restore [/custom/backup/dir/custombackupname.zip]
#
restore() {
  if [[ ! -f .env ]] || [[ ! -f ambientcast.env ]]; then
    echo "AmbientCast hasn't been installed yet on this server."
    echo "You should run './docker.sh install' first before restoring."
    exit 1
  fi

  if ask "Restoring will remove any existing AmbientCast installation data, replacing it with your backup. Continue?" Y; then
    if [[ $1 != "" ]]; then
      local BACKUP_PATH BACKUP_DIR BACKUP_FILENAME BACKUP_EXT
      BACKUP_PATH=$(readlink -f ${1:-"./backup.tar.gz"})
      BACKUP_DIR=$(dirname -- "$BACKUP_PATH")
      ...
      docker-compose run --rm web -- ambientcast_restore "/var/ambientcast/backups/${BACKUP_FILENAME}" "$@"
      ...
      docker volume rm -f $(docker volume ls | grep -v "ambientcast_backups" | awk 'NR>1 {print $2}')

      docker-compose run --rm web -- ambientcast_restore "$@"
...
restore-legacy() {
  ...
  if [ -f "$BACKUP_PATH" ]; then
    ...
    docker volume rm ambientcast_db_data ambientcast_station_data
    docker volume create ambientcast_db_data
    docker volume create ambientcast_station_data

    docker run --rm -v "$BACKUP_DIR:/backup" \
      -v ambientcast_db_data:/ambientcast/db \
      -v ambientcast_station_data:/ambientcast/stations \
...
uninstall() {
  if ask "This operation is destructive and will wipe your existing Docker containers. Continue?" N; then
    ...
    echo "All AmbientCast Docker containers and volumes were removed."
...
#
up() {
  echo "Starting up AmbientCast services..."
  docker-compose up -d
}

down() {
  echo "Shutting down AmbientCast services..."
  docker-compose down
}

...
final class ConfigWriter implements EventSubscriberInterface
{
    ...
    public function writeHeaderFunctions(WriteLiquidsoapConfiguration $event): void
    {
        if (!$event->isForEditing()) {
            $event->prependLines(
                [
                    '# WARNING! This file is automatically generated by AmbientCast.',
        ...
        $event->appendBlock(
            <<<EOF
            ambientcast_api_url = "{$stationApiUrl}"
            ambientcast_api_key = "{$stationApiAuth}"
            
            def ambientcast_api_call(~timeout_ms=2000, url, payload) =
                full_url = "#{ambientcast_api_url}/#{url}"
                ...
                try
                    response = http.post(full_url,
                        headers=[
                            ...
                            ("User-Agent", "Liquidsoap AmbientCast"),
                            ("X-Liquidsoap-Api-Key", "#{ambientcast_api_key}")
                        ],
        ...
        if ($mediaStorageLocation->isLocal()) {
            ...
            $event->appendBlock(
                ...
                def ambientcast_media_protocol(~rlog=_,~maxtime=_,arg) =
                    ["#{station_media_dir}/#{arg}"]
                end
                
                add_protocol(
                    "media",
                    ambientcast_media_protocol,
                    doc="Pull files from AmbientCast media directory.",
                    syntax="media:uri"
                )
                EOF
            );
            ...
            $event->appendBlock(
                <<<EOF
                def ambientcast_media_protocol(~rlog=_,~maxtime,arg) =
                    timeout_ms = 1000 * (int_of_float(maxtime) - int_of_float(time()))
                    
                    j = json()
                    j.add("uri", arg)
                    
                    [ambientcast_api_call(timeout_ms=timeout_ms, "cp", json.stringify(j))]
                end
    ...
    public function writePreBroadcastConfiguration(WriteLiquidsoapConfiguration $event): void
    {
        ...
        $event->appendBlock(
            ...
            # Send metadata changes back to AmbientCast
            last_title = ref("")
            last_artist = ref("")
